...
main(int argc, char *argv[])
{
    ...
    mlir_aie_acquire_lock(_xaie, 1, 3, 0, 0, 0); // Should this part of setup???

    for (int bd=0;bd<1024;bd++) {
        mlir_aie_write_buffer_a(_xaie, bd, 1);
        mlir_aie_write_buffer_b(_xaie, bd, 1);
    ...
    mlir_aie_check("Before release lock:", mlir_aie_read_buffer_b(_xaie, 5), 1,
                   errors);
    
    

    printf("Release input buffer lock.\n");
    mlir_aie_release_lock(_xaie, 1, 3, 0, 1, 0);

    int tries = 1;
    printf("Waiting to acquire output lock for read ...\n");
    while (tries < 1000 && !mlir_aie_acquire_lock(_xaie, 1, 4, 0, 1, 0)) {
      tries++;
    }
