...
#define DMA_COUNT 256
    ...
    int *mem_ptr1 =
      mlir_aie_mem_alloc(_xaie, 1, 0x5000 + 0x020100000000LL, DMA_COUNT+1);


    int header1=generateHeader(127,31,4,0xc);
    int header2=generateHeader(127,31,5,0xd);
    ...
    mlir_aie_release_lock(_xaie, 7, 2, 1, 1, 0);
    mlir_aie_release_lock(_xaie, 7, 4, 1, 1, 0);

    int b721=mlir_aie_acquire_lock(_xaie, 7, 2, 1, 0, 0);
    int b7211=mlir_aie_acquire_lock(_xaie, 7, 2, 1, 1, 0);
    int b741=mlir_aie_acquire_lock(_xaie, 7, 4, 1, 0, 0);
    int b7411=mlir_aie_acquire_lock(_xaie, 7, 4, 1, 1, 0);
    printf("Lock7_2_1:%d\n", b721);
    printf("Lock7_2_1_1:%d\n", b7211);
    printf("Lock7_4_1:%d\n", b741);
    printf("Lock7_4_1_1:%d\n", b7411);

    int tries = 1;
    int tries1 = 1;
    printf("Waiting to acquire output lock for read ...\n");
    while ((tries < 1000) && (!(mlir_aie_acquire_lock(_xaie, 7, 2, 1, 0, 0)))) {
      tries++;
    }
    while (tries1 < 5000 && (!mlir_aie_acquire_lock(_xaie, 7, 4, 1, 0, 0))) {
      tries1++;
    }
    ...
    int a721=mlir_aie_acquire_lock(_xaie, 7, 2, 1, 0, 0);
    int a7211=mlir_aie_acquire_lock(_xaie, 7, 2, 1, 1, 0);
    ...
    for (int bd=0;bd<DMA_COUNT;bd++) {
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf72_0(_xaie, bd), 72,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf74_0(_xaie, bd), 74,
                   errors);
    }
