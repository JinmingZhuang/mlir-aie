...
{
AieRC RC = XAie_LoadElf(&(ctx->DevInst), XAie_TileLoc(7,2), (const char*)"core_7_2.elf",0);
if (RC != XAIE_OK)
...
XAie_DmaSetNextBd(&(dma_tile72_bd1),  /* nextbd */ 1,  /* enableNextBd */ 1);
...
XAie_DmaChannelPushBdToQueue(&(ctx->DevInst), XAie_TileLoc(7,2), /* ChNum */1, /* dmaDir */ DMA_S2MM, /* BdNum */1);
XAie_DmaChannelEnable(&(ctx->DevInst), XAie_TileLoc(7,2), /* ChNum */ 1, /* dmaDir */ DMA_S2MM);
...
XAie_StrmConnCctEnable(&(ctx->DevInst), XAie_TileLoc(x,y), SOUTH, 1, NORTH, 1);
...
u32 value; auto rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(7,2), a_offset + (index*4), &value);
  return value;
}
void mlir_aie_write_buffer_a(aie_libxaie_ctx_t* ctx, int index, int32_t value) {
  int32_t int_value = value;
u32 rc =    XAie_DataMemWrWord(&(ctx->DevInst), XAie_TileLoc(7,2), a_offset + (index*4), int_value);
}
const int b_offset = 8192;
int32_t mlir_aie_read_buffer_b(aie_libxaie_ctx_t* ctx, int index) {
u32 value; auto rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(7,2), b_offset + (index*4), &value);
  return value;
}
void mlir_aie_write_buffer_b(aie_libxaie_ctx_t* ctx, int index, int32_t value) {
  int32_t int_value = value;
u32 rc =    XAie_DataMemWrWord(&(ctx->DevInst), XAie_TileLoc(7,2), b_offset + (index*4), int_value);
}
const int acc_offset = 4096;
int32_t mlir_aie_read_buffer_acc(aie_libxaie_ctx_t* ctx, int index) {
u32 value; auto rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(7,3), acc_offset + (index*4), &value);
  return value;
}
void mlir_aie_write_buffer_acc(aie_libxaie_ctx_t* ctx, int index, int32_t value) {
  int32_t int_value = value;
u32 rc =    XAie_DataMemWrWord(&(ctx->DevInst), XAie_TileLoc(7,3), acc_offset + (index*4), int_value);
}
const int c_offset = 8192;
int32_t mlir_aie_read_buffer_c(aie_libxaie_ctx_t* ctx, int index) {
u32 value; auto rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(7,3), c_offset + (index*4), &value);
  return value;
}
void mlir_aie_write_buffer_c(aie_libxaie_ctx_t* ctx, int index, int32_t value) {
  int32_t int_value = value;
u32 rc =    XAie_DataMemWrWord(&(ctx->DevInst), XAie_TileLoc(7,3), c_offset + (index*4), int_value);
}
