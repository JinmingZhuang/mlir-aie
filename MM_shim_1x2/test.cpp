...
#define DMA_COUNT 1024
    int *mem_ptr0 =
      mlir_aie_mem_alloc(_xaie, 0, 0x2000 + 0x020100000000LL, DMA_COUNT+1);
    int *mem_ptr1 =
      mlir_aie_mem_alloc(_xaie, 1, 0x4000 + 0x020100000000LL, DMA_COUNT+1);
    int *mem_ptr2 =
    ...
    int header0=generateHeader(127,31,0,0x0);
    int header1=generateHeader(127,31,1,0x1);
    int header2=generateHeader(127,31,2,0x2);
    int header3=generateHeader(127,31,3,0x3);
    ...
    mlir_aie_release_lock(_xaie, 7, 0, 3, 1, 0);
    ...
    for (int bd=0;bd<DMA_COUNT;bd++) {
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf73_0(_xaie, bd), 1,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf73_1(_xaie, bd), 1,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf74_0(_xaie, bd), 2,
                       errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf74_1(_xaie, bd), 2,
               errors);
