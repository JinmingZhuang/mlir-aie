...
//
  ...
  AIE.flow(%t70, "DMA" : 0, %t71, "North" : 3)
  

  %sw72 = AIE.switchbox(%t72) {
    %tmsel0 = AIE.amsel<1> (0) // <arbiter> (mask). mask is msel_enable
    %tmsel1 = AIE.amsel<2> (0) // <arbiter> (mask). mask is msel_enable
  ...
  %sw73 = AIE.switchbox(%t73) {
    //AIE.connect<"South" : 3, "North" : 3>
    %tmsel0 = AIE.amsel<1> (0) // <arbiter> (mask). mask is msel_enable
    %tmaster1 = AIE.masterset(North : 3, %tmsel0)
    AIE.packetrules(South : 3)  {
      AIE.rule(0x1f, 0xd, %tmsel0) // (mask, id)
    }
  }
