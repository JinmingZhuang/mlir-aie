...
//
    ...
    mlir_aie_clear_tile_memory(_xaie, 6, 2);
    ...
    for (int bd=0;bd<DMA_COUNT;bd++) {
        mlir_aie_write_buffer_buf62_0(_xaie, bd, 1);
        mlir_aie_write_buffer_buf62_1(_xaie, bd, 2);
        mlir_aie_write_buffer_buf62_2(_xaie, bd, 1);
        mlir_aie_write_buffer_buf62_3(_xaie, bd, 2);
    }


    for (int bd=0;bd<DMA_COUNT;bd++) {
        mlir_aie_write_buffer_buf63_2(_xaie, bd, 1);
    }
    ...
    mlir_aie_release_lock(_xaie, 6, 2, 4, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 2, 5, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 2, 6, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 2, 7, 0, 0);

    mlir_aie_release_lock(_xaie, 6, 3, 0, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 3, 1, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 3, 2, 0, 0);

    mlir_aie_release_lock(_xaie, 6, 4, 0, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 4, 1, 0, 0);
    mlir_aie_release_lock(_xaie, 6, 4, 2, 0, 0);

    usleep(sleep_u);
    mlir_aie_print_tile_status(_xaie, 6, 3);
    mlir_aie_print_tile_status(_xaie, 6, 4);

    int tries0 = 1;
    int tries1 = 1;

    while ((tries1 < 1000) && (!mlir_aie_acquire_lock(_xaie, 6, 4, 2, 1, 0))) {
      tries1++;
    }
    ...
    for (int bd=0;bd<DMA_COUNT;bd++) {
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf62_0(_xaie, bd), 1,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf62_1(_xaie, bd), 2,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf62_2(_xaie, bd), 1,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf62_3(_xaie, bd), 2,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf63_2(_xaie, bd), 1,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf63_3(_xaie, bd),33,
                   errors);
        mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf64_2(_xaie, bd), 161,
                   errors);
    }

    mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf63_0(_xaie, 1023), 1,
                   errors);
    mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf63_1(_xaie, 1023), 1,
               errors);
    mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf64_0(_xaie, 1023), 2,
                   errors);
    mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf64_1(_xaie, 1023), 2,
               errors);
